import { Router } from 'express';
import { db } from '../db.js';
import { requireAuth } from '../middleware/auth.js';

const router = Router();

// GET /api/messages/conversations — list of people I've messaged with + last message
router.get('/conversations', requireAuth, async (req, res, next) => {
 try {
  const uid = req.userId;
  const rows = await db.all(`
    SELECT
      CASE WHEN m.fromUserId = ? THEN m.toUserId ELSE m.fromUserId END as otherId,
      m.text as lastMessage, m.createdAt as time, m.fromUserId, m.isRead
    FROM messages m
    WHERE m.fromUserId = ? OR m.toUserId = ?
    ORDER BY m.createdAt DESC
  `, [uid, uid, uid]);

  const seen = new Map();
  for (const r of rows) {
    if (!seen.has(r.otherId)) seen.set(r.otherId, r);
  }

  const conversations = await Promise.all([...seen.values()].map(async r => {
    const user = await db.get('SELECT id, fullName, email, role FROM users WHERE id = ?', [r.otherId]);
    const unreadCount = (await db.get(`
      SELECT COUNT(*) c FROM messages WHERE fromUserId = ? AND toUserId = ? AND isRead = FALSE
    `, [r.otherId, uid])).c;
    return {
      userId: r.otherId,
      user,
      lastMessage: r.lastMessage,
      time: r.time,
      isSentByMe: r.fromUserId === uid,
      unreadCount,
    };
  }));

  res.json(conversations);
 } catch (error) { next(error); }
});

// GET /api/messages/:userId — full thread with a specific user
router.get('/:userId', requireAuth, async (req, res, next) => {
 try {
  const uid = req.userId;
  const otherId = Number(req.params.userId);
  const other = await db.get('SELECT id, fullName, email, role FROM users WHERE id = ?', [otherId]);
  if (!other) return res.status(404).json({ error: 'İstifadəçi tapılmadı.' });

  const messages = await db.all(`
    SELECT * FROM messages
    WHERE (fromUserId = ? AND toUserId = ?) OR (fromUserId = ? AND toUserId = ?)
    ORDER BY createdAt ASC
  `, [uid, otherId, otherId, uid]);

  await db.run(`UPDATE messages SET isRead = TRUE WHERE fromUserId = ? AND toUserId = ? AND isRead = FALSE`, [otherId, uid]);

  res.json({ user: other, messages });
 } catch (error) { next(error); }
});

// POST /api/messages — send a message { toUserId, text }
router.post('/', requireAuth, async (req, res, next) => {
 try {
  const { toUserId, text } = req.body || {};
  if (!toUserId || !text || !text.trim()) {
    return res.status(400).json({ error: 'Alıcı və mesaj mətni tələb olunur.' });
  }
  const target = await db.get('SELECT id FROM users WHERE id = ?', [toUserId]);
  if (!target) return res.status(404).json({ error: 'Alıcı tapılmadı.' });

  const info = await db.run(`INSERT INTO messages (fromUserId, toUserId, text) VALUES (?, ?, ?) RETURNING id`,
    [req.userId, toUserId, text.trim()]);
  const row = await db.get('SELECT * FROM messages WHERE id = ?', [info.lastInsertRowid]);
  res.status(201).json(row);
 } catch (error) { next(error); }
});

export default router;
